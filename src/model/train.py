...
if __name__ == '__main__':
    ...
    train_data = np.stack([np.array(Image.open("../auto_get_image_by_selenium/img_processed/" + str(index) + ".jpg"))/255.0 for index in range(1, 11001, 1)])
    traincsv = open('../auto_get_image_by_selenium/label/label.csv', 'r', encoding='utf8')
    ...
    vali_data = np.stack([np.array(Image.open("../download_img/processed_image/"+ str(index) + ".jpg"))/255.0 for index in range(1, 5001, 1)])
    valicsv = open('../download_img/label5000.csv', 'r', encoding ='utf8')
    ...
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=2, save_best_only=False, mode='min')
    earlystop = EarlyStopping(monitor='val_loss', patience=3, verbose=1, mode='min')
    tensorBoard = TensorBoard(log_dir='./logs', histogram_freq=1)
    callbacks_list = [tensorBoard, earlystop, checkpoint]
    model.fit(train_data, train_label, batch_size=50, epochs=40, validation_split=0.2, callbacks=callbacks_list, verbose=2)
